...
def is_below_value(
    ...
    # Calculate if series is below value
    result = series <= value
    ...
    result_column = f"{column}_below_{value_str}"
...
def test():
    """
    Test function for the is_below_value indicator.
    
    This function creates a simple test DataFrame with price data
    and tests the is_below_value functionality.
    ...
    try:
        ...
        # Expected results for is_below_value
        expected_integer = [1, 1, 0, 0, 1, 1]  # Note: 100 <= 100 is True
        expected_boolean = [True, True, False, False, True, True]
        
        # Create a DataFrame with test data
        df = pd.DataFrame(data)
        
        # Test with as_int=True (default)
        result_int = is_below_value(df, 'price', test_value)
        
        # Test with as_int=False
        result_bool = is_below_value(df, 'price', test_value, as_int=False)
        
        # Print results
        print("Test DataFrame:")
        print(df)
        
        print(f"\nIs Below Value {test_value} (as integers):")
        print(result_int)
        print(f"Expected: {expected_integer}")
        print(f"Matches expected: {list(result_int[f'price_below_{test_value}']) == expected_integer}")
        
        print(f"\nIs Below Value {test_value} (as booleans):")
        print(result_bool)
        print(f"Expected: {expected_boolean}")
        print(f"Matches expected: {list(result_bool[f'price_below_{test_value}']) == expected_boolean}")
        
        # Test with decimal value
        decimal_value = 102.5
        result_decimal = is_below_value(df, 'price', decimal_value)
        print(f"\nIs Below Value {decimal_value} (decimal value):")
